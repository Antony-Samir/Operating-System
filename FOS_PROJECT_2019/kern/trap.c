...
void page_fault_handler(struct Env* curenv, uint32 fault_va)
{
	...
	if (env_page_ws_get_size(curenv) >= curenv->page_WS_max_size)
	{
		...
		{//modify Index by CLOCK logic

			while (1==1)
			{
				Var = env_page_ws_get_virtual_address(curenv, Index);
				uint32 Entry_Table;
				Ptr_Page_Table= NULL;
				get_page_table(curenv->env_page_directory, (void*)Var, &Ptr_Page_Table);
				Entry_Table = Ptr_Page_Table[PTX(Var)];

				if ((Entry_Table & PERM_USED) == 0)
					break;
				else
					Ptr_Page_Table[PTX(Var)] &= ~(PERM_USED);

				Index = (Index + 1) % curenv->page_WS_max_size;
			}
		}
		//remove page to be replaced regardless of the algorithm used
		void* Var = (void*)env_page_ws_get_virtual_address(curenv, Index);
		Ptr_Page_Table = NULL;
		get_page_table(curenv->env_page_directory, Var, &Ptr_Page_Table);
		struct Frame_Info* frame_info=
				get_frame_info(curenv->env_page_directory, Var, &Ptr_Page_Table);

		if (Ptr_Page_Table[PTX(Var)] & PERM_MODIFIED)
		{
			pf_update_env_page(curenv, Var, frame_info);
		}
		unmap_frame(curenv->env_page_directory, Var);
		env_page_ws_clear_entry(curenv, Index);
	}
	//HAPPENS IN ALL CASES
	while (!curenv->__uptr_pws[Index].empty)
	{
		Index = (Index + 1) % curenv->page_WS_max_size;
	}
	struct Frame_Info* frame_info;
	...
	map_frame(curenv->env_page_directory, frame_info, (void*)fault_va, PERM_USER | PERM_WRITEABLE | PERM_PRESENT | PERM_USED);
	uint32 X = pf_read_env_page(curenv, (void*)fault_va);
	if (X == 0)
	{
		env_page_ws_set_entry(curenv, Index, fault_va);
	}
	else if (X == E_PAGE_NOT_EXIST_IN_PF)
	{
		...
		else
		{
			...
			panic("NOT STACK");//Hereeeeeeeeeeeeeeeed
		}
	}
