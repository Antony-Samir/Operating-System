...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32  va = (uint32) kmalloc(PAGE_SIZE);//Virtual Address
	uint32 Dir = kheap_physical_address(va); //Directory
	Dir = Dir | PERM_WRITEABLE;
	Dir = Dir | PERM_PRESENT;
	Dir = Dir | PERM_USER;
	ptr_page_directory[PDX(virtual_address)] = Dir;
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 Var = virtual_address;
	uint32 *Ptr_Page_Table;
	uint32 Free_Pages; //Will Use in Step 3
	// Step 1 Free All Pages
	for(int i = 0; i < size; i =i + PAGE_SIZE)
	{
		pf_remove_env_page(e, Var);
		Var = Var + PAGE_SIZE;
	}
	// Step 2 Free Only Specific Pages
	Var = virtual_address;
	for(int i = 0; i < size; i =i + PAGE_SIZE)
	{
		for(int j = 0;j < e->page_WS_max_size;j++)
		{
			if(e->ptr_pageWorkingSet[j].virtual_address == Var)
			{
				env_page_ws_clear_entry(e, j);
				unmap_frame(e->env_page_directory,(void *)Var);
				break;
			}
		}
		Var = Var+ PAGE_SIZE;
	}
	// Step 3 Remove Empty Page Tables
	// Loop on the Tables to determine which tables should remove
	Var = ROUNDDOWN(virtual_address,PAGE_SIZE*1024);
	for(int i = Var; i <= (virtual_address+size); i= i+(PAGE_SIZE*1024))
	{
